...
export default function PlayerLookup() {
  ...
  const {
    ...
    error,
    refetch,
  } = usePlayerSnapshots(selectedPlayer);
  const {
    data: currentStatsData,
    isLoading: isLoadingCurrent,
    error: errorCurrent,
  } = usePlayerCurrentStats(selectedPlayer);
      ...
      <div className="container">
        ...
        <div className="card">
          <p style={{ marginBottom: '1rem' }}>
            Select a player to view their detailed profile and statistics:
          </p>
  ...
  if (!snapshots || snapshots.length === 0) {
    return (
      <div className="container">
        <h1 style={{ fontSize: '2rem', marginBottom: '1.5rem' }}>Player Lookup</h1>
        <PlayerSearch players={playerList} onSelect={handlePlayerSelect} />
        <ErrorMessage title="No Data Found" message="No snapshot data found for this player." />
      </div>
    );
  }
  ...
  return (
    <div className="container">
      ...
      <div className="info-message" style={{ marginBottom: '2rem' }}>
        <strong>{latest.ign}</strong> ({latest.display_name}) • Discord:{' '}
        {latest.discord_name || latest.discord_id}
      </div>
      ...
      {isLoadingCurrent ? (
        ...
        <div className="warning-message" style={{ marginBottom: '2rem' }}>
          ...
          <p>
            This player's live stats will appear here after their first update in Egg Inc (usually
            within 24 hours).
          </p>
        </div>
      ) : (
        <div className="card" style={{ marginBottom: '2rem', overflowX: 'auto' }}>
          <div className="info-message" style={{ marginBottom: '1rem' }}>
            Last updated: <strong>{formatLastUpdated(currentStatsData.lastUpdated)}</strong>{' '}
          ...
          <table>
            ...
            <tbody>
              ...
              <tr>
                ...
                <td>
                  {currentStatsData.player.pe != null
                    ? formatInteger(currentStatsData.player.pe)
                    : 'N/A'}
                </td>
              </tr>
              <tr>
                ...
                <td>
                  {currentStatsData.player.te != null
                    ? formatInteger(currentStatsData.player.te)
                    : 'N/A'}
                </td>
              </tr>
              <tr>
                ...
                <td>
                  {currentStatsData.player.mer != null && !isNaN(currentStatsData.player.mer)
                    ? currentStatsData.player.mer.toFixed(2)
                    : 'N/A'}
                </td>
              </tr>
              <tr>
                ...
                <td>
                  ...
                  isFinite(currentStatsData.player.jer)
                    ? currentStatsData.player.jer.toFixed(2)
                    : 'N/A'}
                </td>
              </tr>
              <tr>
                ...
                <td>
                  {currentStatsData.player.num_prestiges != null
                    ? formatInteger(currentStatsData.player.num_prestiges)
                    : 'N/A'}
                </td>
              </tr>
              <tr>
      ...
      >
        ...
        <div className="card" style={{ maxHeight: '300px', overflowY: 'auto' }}>
          ...
          <ul style={{ listStyle: 'none', padding: 0, margin: 0 }}>
            {Array.from(
              new Set([
                ...(currentStatsData?.player?.discord_name
                  ? [currentStatsData.player.discord_name]
                  : []),
              ])
            )
        ...
        <div className="card" style={{ maxHeight: '300px', overflowY: 'auto' }}>
          ...
          <ul style={{ listStyle: 'none', padding: 0, margin: 0 }}>
            {Array.from(
              new Map(
                [
                  ...(currentStatsData?.player?.ign
                    ? [{ name: currentStatsData.player.ign, date: new Date().toISOString() }]
                    : []),
                  ...snapshots
        ...
        <div className="card" style={{ maxHeight: '300px', overflowY: 'auto' }}>
          ...
          <ul style={{ listStyle: 'none', padding: 0, margin: 0 }}>
            {Array.from(
              new Map(
                [
                  ...(currentStatsData?.player?.display_name
                    ? [
                        {
                          name: currentStatsData.player.display_name,
                  ...
                  ...snapshots
                    ...
                    .filter(
                      (item): item is { name: string; date: string } =>
      ...
      >
        <div className="metric-card">
          ...
          <div
            ...
            style={{ fontSize: '1.5rem', color: ebGain > 0 ? '#4ade80' : '#f87171' }}
          >
            {ebGain > 0 ? '+' : ''}
            {bigNumberToString(ebGain)}
          </div>
        </div>
        <div className="metric-card">
          ...
          <div
            className="metric-value"
            style={{ fontSize: '1.5rem', color: seGain > 0 ? '#4ade80' : '#f87171' }}
          >
            {seGain > 0 ? '+' : ''}
            {bigNumberToString(seGain)}
          </div>
        </div>
        <div className="metric-card">
          ...
          <div
            className="metric-value"
            style={{ fontSize: '1.5rem', color: peGain > 0 ? '#4ade80' : '#f87171' }}
          >
            {peGain > 0 ? '+' : ''}
            {formatInteger(peGain)}
          </div>
        </div>
      </div>
